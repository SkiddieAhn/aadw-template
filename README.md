...
AI는 사람이 설계한 만큼 잘 만든다. 프롬프트 한 줄로 좋은 결과를 기대하는 건 AI를 믿는 게 아니라 사람의 설계 부재를 AI에게 떠넘기는 것이다. AADW는 그 반대를 택한다. **사람이 설계와 검증에 집중하고, AI는 그 틀 안에서 실행한다.**
...
| 문서 | 설명 |
|---|---|
| PRD | 이 라이브러리/모듈이 해결하는 문제 상황과 핵심 기능을 정의한다 (비전공자도 이해 가능한 기획 수준) |
| Architecture | 코드베이스의 전체 구조와 실행 흐름을 top-down으로 설명한다 (유즈케이스 및 시퀀스 다이어그램 포함) |
| Spec | 호출/구현 시 지켜야 하는 구조와 계약을 정의한다 (클래스 다이어그램, 표, 계약 조건 포함) |
...
- 호출/구현되는 클래스 위주로 **Mermaid 클래스 다이어그램 + 필드/메서드 표 + 입출력 표**를 제공한다.
- 구성:

  **섹션 1 — 포커스 범위**
  - 디렉토리 구조에서 ★로 어떤 클래스를 다루는지 명시
  - 나머지를 다루지 않는 이유 한 줄 설명

  **섹션 2 — 클래스 관계 개요**
  - 전체 클래스의 상속·소유 관계를 한 Mermaid 다이어그램으로 표현
  - 다이어그램 규칙:
    - 클래스 내부에는 **필드만** 표시. 메서드는 적지 않는다
    - 추상 클래스·인터페이스에는 `<<abstract>>` 마커를 붙인다
    - 화살표 종류: 상속 `<|--`, composition `*--`, association `-->`, dependency `..>`
    - 관계 레이블은 필드명이나 조건(예: `graph-rag 시`)을 간결하게 붙인다
  - 다이어그램 아래에 세 가지 표를 작성한다:
    1. **클래스/인터페이스 설명 표** (클래스명 / 위치 / 한 줄 설명) — 다이어그램에 등장한 모든 클래스 포함
    2. **화살표 읽는 법 표** (표기 / 의미) — 다이어그램에 사용된 화살표 종류만
    3. **레이블 설명 표** (레이블 / 설명) — 다이어그램에 붙인 레이블 중 설명이 필요한 것만

  **섹션 3+ — 클래스별 상세**
  - 포커스 범위의 ★ 클래스마다 한 섹션씩 작성
  - 각 섹션 구성:
    1. 클래스 다이어그램 (Mermaid) — 해당 클래스와 직접 연관된 클래스만
    2. 필드/메서드 표 (구분 / 이름 / 설명)
    3. 주요 메서드 입출력 표 (키 / 설명) — 입출력이 복잡한 메서드만 작성
    4. 계약 표:
       - 입력 제약조건: 유효한 입력의 전제 조건
       - 반환 보장: 성공 시 반환값의 형태와 보장 사항
       - 예외/에러 조건: 어떤 상황에서 예외가 발생하거나 빈 결과를 반환하는가
    5. 사용 예시: 이 클래스를 실제로 어떻게 호출하는가 (내부 구현이 아닌 호출자 관점)
